    ...
    def on_event(self, f):        
        ...
        def set_action(self, robot_id, action_number):
            if action_number == 0:
                ...
                self.position(robot_id, (self.cur_ball[X]-0.13), (self.cur_ball[Y]+0.1))
                ...
                self.position(robot_id, self.cur_ball[X] + 10*delta[X], self.cur_ball[Y] + 10*delta[Y])
...
##############################################################################
            ...
            if self.global_step % self.stats_steps == 0: # plot the statics
                stats = [self.reward_sum, self.agent.avg_q_max / float(self.step), self.step,
                             self.agent.avg_loss / float(self.step), self.score_op_sum]

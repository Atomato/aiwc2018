...
##############################################################################    
    ...
    def get_reward(self, reset_reason, i):
        pre_potential = helper.dipole_potential(self.pre_ball[X], self.pre_ball[Y], 2.1, 30)
        cur_potential = helper.dipole_potential(self.cur_ball[X], self.cur_ball[Y], 2.1, 30)
        potential_rew = cur_potential - pre_potential
        # Reset
        if(reset_reason != NONE):
            potential_rew = 0
        self.printConsole('     potential reward ' + str(i) + ': ' + str(potential_rew))
        ...
        if (abs(self.cur_ball[X]) < 1.85) and (abs(self.cur_ball[Y]) < 1.3):        
            ...
            if self.cur_my_posture[i][TOUCH]:
                touch_rew -= 0.01
                touch_case = 3
        self.printConsole('             touch reward ' + str(i) + ': ' + str(touch_rew) +
            ' (case: ' + str(touch_case) + ')')

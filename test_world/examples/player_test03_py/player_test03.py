...
##############################################################################
        def init_variables(self, info):
            ...
            self.state_size = 32 # the number of possible states
            ...
            self.stats_steps = 50000 # for tensorboard
            self.reward_sum = 0
            return
##############################################################################
...
##############################################################################
            ...
            # Reward
            reward = 3*math.exp(-10*(helper.distance(self.cur_my_posture[0][X], self.cur_ball[X], self.cur_my_posture[0][Y], self.cur_ball[Y])/20))
            ...
            if(received_frame.reset_reason == SCORE_MYTEAM):
                reward += 500
                self.printConsole("score myteam, reward is " + str(reward))
            if(received_frame.reset_reason == SCORE_OPPONENT):
                reward -= 500
                self.printConsole("score opponet, reward is " + str(reward))
            # Reset

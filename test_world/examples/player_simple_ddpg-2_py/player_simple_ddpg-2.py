...
# exploration with rule-based policy
...
##############################################################################
            ...
            if np.random.rand() <= self.agent.epsilon:
                ...
                self.agent.noise = self.agent.ou_noise(self.agent.noise, 0.0, 0.15, 0.2)
                self.printConsole("noise: " + str(self.agent.noise[:2]))
                self.wheels = self.wheels + self.agent.noise * self.agent.action_max * max(self.agent.epsilon, 0)

...
class DDPGAgent:
    def __init__(self, state_dim, action_dim, action_max, action_min):
        ...
        self.state_ph = tf.placeholder(dtype=tf.float32, shape=[None, self.state_dim * self.history_size])
        self.reward_ph = tf.placeholder(dtype=tf.float32, shape=[None])
        self.next_state_ph = tf.placeholder(dtype=tf.float32, shape=[None, self.state_dim * self.history_size])

...
##############################################################################
        def init_variables(self, info):
            ...
            self.state_dim = 23 # 10 robot, 1 ball, theta
            self.history_size = 2 # frame history size
            self.action_dim = 2 # 2                    
            
            self.arglist = Argument()
            self.obs_shape_n = [(self.state_dim * self.history_size,) for _ in range(self.number_of_robots)] # state dimenstion
            self.action_space = [spaces.Discrete(self.action_dim * 2 + 1) for _ in range(self.number_of_robots)]
            self.trainers = self.get_trainers(self.number_of_robots, self.obs_shape_n, self.action_space, self.arglist)
...
##############################################################################    
    ...
    def pre_processing(self, i):
        ...
        for k in range(self.number_of_robots):
            if k is i: continue
            relative_my_team.append(helper.rot_transform(self.cur_my_posture[i][X], 
                self.cur_my_posture[i][Y], -self.cur_my_posture[i][TH], 
                self.cur_my_posture[k][X], self.cur_my_posture[k][X]))

        relative_op_team = []
        for k in range(self.number_of_robots):
            relative_op_team.append(helper.rot_transform(self.cur_my_posture[i][X], 
                self.cur_my_posture[i][Y], -self.cur_my_posture[i][TH], 
                self.cur_op_posture[k][X], self.cur_op_posture[k][X]))
        ...
        return np.concatenate([relative_ball] + [relative_goal] + relative_my_team + relative_op_team + [[theta]])

...
##############################################################################
        def init_variables(self, info):
            ...
            self.distances = [[i for i in range(self.number_of_robots)], [i for i in range(self.number_of_robots)]] # distances to the ball
            self.idxs = [[i for i in range(self.number_of_robots)], [i for i in range(self.number_of_robots)]]
            self.deadlock_cnt = 0
    ...
    def on_event(self, f):        
        ...
        def midfielder(self, robot_id):
            ...
            if self.distances[MY_TEAM][robot_id] < 0.5:
                # near
                self.wheels[2*robot_id:2*robot_id + 2] = self.agent.get_action(np.reshape(self.histories[robot_id], -1)) # ddpg action
                self.printConsole("robot id " + str(robot_id) + " distance: " + str(self.distances[MY_TEAM][robot_id]))
...
##############################################################################
            ...
            # update state
            for i in range(self.number_of_robots):
                next_state = [round(self.cur_ball[X], 2)- round(self.cur_my_posture[i][X], 2), 
                    round(self.cur_ball[Y], 2) - round(self.cur_my_posture[i][Y], 2), 
                    round(self.cur_my_posture[i][TH]/(2*math.pi), 2)]
                next_state = np.reshape([next_state], (self.state_dim, 1))
                self.histories[i] = np.append(next_state, self.histories[i][:, :self.history_size-1], axis=1)

...
##############################################################################
        def init_variables(self, info):
            ...
            self.control_robot_num = [0]
            self.obs_shape_n = [(self.state_dim * self.history_size,) for _ in range(len(self.control_robot_num))] # state dimenstion
            self.action_space = [spaces.Discrete(self.action_dim * 2 + 1) for _ in range(len(self.control_robot_num))]
            self.trainers = self.get_trainers(len(self.control_robot_num), self.obs_shape_n, self.action_space, self.arglist)
            ...
            self.agent_rewards = [[0.0] for _ in range(len(self.control_robot_num))]  # individual agent reward
            ...
            self.obs_n = [np.zeros([self.state_dim * self.history_size]) for _ in range(len(self.control_robot_num))] # histories
            self.train_step = 0
            self.wheels = np.zeros(self.number_of_robots*2)
            self.action_n = [np.zeros(self.action_dim * 2 + 1) for _ in range(len(self.control_robot_num))]
...
##############################################################################    
    ...
    def get_reward(self, reset_reason):
        ...
        reward = -helper.distance2(self.cur_ball[X], self.cur_my_posture[self.control_robot_num[0]][X], 
            self.cur_ball[Y], self.cur_my_posture[self.control_robot_num[0]][Y])

        if self.cur_my_posture[self.control_robot_num[0]][TOUCH]:
            reward += 100
    ...
    def pre_processing(self, i):
        ...
        processed_state = helper.rot_transform(self.cur_my_posture[self.control_robot_num[i]][X], 
            self.cur_my_posture[self.control_robot_num[i]][Y], -self.cur_my_posture[self.control_robot_num[i]][TH], 
...
##############################################################################
            # self.printConsole("get session: " + str(self.trainers[0].get_cur_session()))
            # Next state
            new_obs_n = [np.zeros([self.state_dim * self.history_size]) for _ in range(len(self.control_robot_num))]
            for i in range(len(self.control_robot_num)):
                ...
                # self.printConsole('theta: ' + str(self.cur_my_posture[self.control_robot_num[i]][TH]/math.pi) + 'pi')

            # Reward
            rew_n = [self.get_reward(received_frame.reset_reason)]
            ...
            self.wheels[0] = self.max_linear_velocity * (self.action_n[0][1]-self.action_n[0][2]+self.action_n[0][3]-self.action_n[0][4])
            self.wheels[1] = self.max_linear_velocity * (self.action_n[0][1]-self.action_n[0][2]-self.action_n[0][3]+self.action_n[0][4])
